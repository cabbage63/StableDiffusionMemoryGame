...
const judgeTriedCards = () => {
  ...
  const paired = triedCards.every(
    (currentValue) => currentValue.card.label == triedCards[0].card.label
  );
...
const printState = () => {
  board.flat().forEach((e) => {
    console.log(
      `[${e.x}, ${e.y}] label: ${e.card.label}, opened: ${e.opened}, tried: ${e.tried}, paired:${e.paired}`
    );
  });
